 ...
 */
export function loadConfig(paths = '.github/checkwise.yml'): CheckwiseCfg {
  // Accept comma-separated paths, trim whitespace
  const pathList = paths.split(',').map(p => p.trim()).filter(Boolean);
  if (pathList.length === 0) {
    throw new Error('No config path(s) provided.');
  }

  let mergedChecklists: any[] = [];
  let mergedOptions: Record<string, any> = {};
  let loadedAny = false;
  ...
  if (!loadedAny || mergedChecklists.length === 0) {
    throw new Error('No valid checklists found in any config file.');
  }

  return mergedOptions && Object.keys(mergedOptions).length > 0
    ? { checklists: mergedChecklists, options: mergedOptions }
